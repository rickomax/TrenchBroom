	...
	**/
	public class MeshMerge
	{
		...
		public struct Face
		{
			...
			public int[] points;
			public Vector2[] uvs;
			// Index of the originating face in the source brush (A or B per from_b).
			// Sub-triangles produced by Build2DFaces inherit the parent face's index.
							...
							if ( ((FaceBVH)current_facebvh).face >= 0 )
							{
								while ( current_facebvh != null )
								{
									if ( ((FaceBVH)current_facebvh).aabb.intersects_ray( face_center, face_normal ) )
									{
										...
										{
											float distance = face_center.Distance( intersection_point );
		...
		/// <summary>
		/// Helper that copies a MeshMerge.Face into a CSGBrush output slot, preserving
		/// per-face metadata (source brush + source face index). When flip is true the
		/// triangle winding is reversed (used by SUBTRACTION on the B side).
		/// </summary>
		private void emit_face( ref CSGBrush r, ref int faces_count, Face src, bool flip = false )
		{
			r.faces[faces_count].vertices = new List<Vector3>( 3 );
			if ( flip )
			{
				r.faces[faces_count].vertices.Add( points[src.points[1]] );
				r.faces[faces_count].vertices.Add( points[src.points[0]] );
				r.faces[faces_count].vertices.Add( points[src.points[2]] );
				r.faces[faces_count].uvs = new Vector2[3] { src.uvs[1], src.uvs[0], src.uvs[2] };
			}
			else
			{
				r.faces[faces_count].vertices.Add( points[src.points[0]] );
				r.faces[faces_count].vertices.Add( points[src.points[1]] );
				r.faces[faces_count].vertices.Add( points[src.points[2]] );
				r.faces[faces_count].uvs = new Vector2[3] { src.uvs[0], src.uvs[1], src.uvs[2] };
			}
			r.faces[faces_count].from_b = src.from_b;
			r.faces[faces_count].source_face_index = src.source_face_index;
			faces_count++;
		}
				...
				case Operation.OPERATION_UNION:
					{
						...
						for ( int i = 0; i < faces_a.Count; i++ )
						{
							...
							{
								emit_face( ref r_merged_brush, ref faces_count, faces_a[i] );
								continue;
							}
						...
						for ( int i = 0; i < faces_b.Count; i++ )
						{
							...
							{
								emit_face( ref r_merged_brush, ref faces_count, faces_b[i] );
								continue;
							}
				...
				case Operation.OPERATION_SUBTRACTION:
					{
						...
						for ( int i = 0; i < faces_b.Count; i++ )
						{
							...
							if ( bvh_inside( ref facebvh_a, max_depth_a, max_alloc_a - 1, i, false ) )
							{
								// Reverse winding so the carved cavity faces inwards.
								emit_face( ref r_merged_brush, ref faces_count, faces_b[i], flip: true );
							}
						}
						Array.Resize( ref r_merged_brush.faces, faces_count );
					}
		...
		public void add_face( Vector3[] points, Vector2[] uvs, bool from_b, int source_face_index = -1 )
		{
			int[] indices = new int[3];
			for ( int i = 0; i < 3; i++ )
			{
				Vector3 vk = new Vector3();
				vk.x = MathF.Round( ((points[i].x + vertex_snap) * 0.31234f) / vertex_snap );
				vk.y = MathF.Round( ((points[i].y + vertex_snap) * 0.31234f) / vertex_snap );
				vk.z = MathF.Round( ((points[i].z + vertex_snap) * 0.31234f) / vertex_snap );
				...
				else
				{
					...
					this.points.Add( points[i] * scale_a );
			...
			face.source_face_index = source_face_index;
