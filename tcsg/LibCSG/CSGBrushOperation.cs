...
public class CSGBrushOperation
{
    ...
    public void merge_brushes(Operation operation, CSGBrush brush_a, CSGBrush brush_b, ref CSGBrush merged_brush, float tolerance = 0.00001f) {
        ...
        for (int i = 0; i < brush_a.faces.Length; i++) {
            if (build2DFaceCollection.build2DFacesA.ContainsKey(i)) {
                ...
                mesh_merge.add_face(points, uvs, false, i);
            }
        }
        ...
        for (int i = 0; i < brush_b.faces.Length; i++) {
            if (build2DFaceCollection.build2DFacesB.ContainsKey(i)) {
                ...
                Vector2[] uvs = new Vector2[3];
                for (int j = 0; j < 3; j++) {
                    points[j] = brush_a.obj.Transform.Local.PointToLocal(brush_b.obj.Transform.World.PointToWorld(brush_b.faces[i].vertices[j]));
                    uvs[j] = brush_b.faces[i].uvs[j];
                }
                mesh_merge.add_face(points, uvs, true, i);
            }
        }
    ...
    void update_faces(ref CSGBrush brush_a, int face_idx_a, ref CSGBrush brush_b, int face_idx_b, ref Build2DFaceCollection collection, float vertex_snap) {
        ...
        Vector3[] vertices_b = {
            brush_a.obj.Transform.Local.PointToLocal(brush_b.obj.Transform.World.PointToWorld(brush_b.faces[face_idx_b].vertices[0])),
            brush_a.obj.Transform.Local.PointToLocal(brush_b.obj.Transform.World.PointToWorld(brush_b.faces[face_idx_b].vertices[1])),
            brush_a.obj.Transform.Local.PointToLocal(brush_b.obj.Transform.World.PointToWorld(brush_b.faces[face_idx_b].vertices[2]))
        };
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (MathF.Abs(plane_a.GetDistance( vertices_b[i])) < distance_tolerance) {
                // In plane.
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (MathF.Abs(plane_b.GetDistance(vertices_a[i])) < distance_tolerance) {
                // In plane.
            } else if (is_point_over) {
                over_count++;
            } else {
