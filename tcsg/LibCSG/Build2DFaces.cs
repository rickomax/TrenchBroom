...
namespace LibCSG{
    ...
    **/
    public class Build2DFaces{
        ...
        /// </summary>
		// Practical tolerance for 2D face splitting on float geometry. 1e-10 is too
		// strict for runtime-generated brush data and causes unstable edge/vertex
		// classification around nearly-coincident intersections.
		private float vertex_tolerance = 1e-8f;
        ...
        /// <summary>
        /// Index of the originating face in the source brush. Sub-triangles produced
        /// during 2D splitting all inherit this index so per-face material/texture
        /// parameters can be looked up after the boolean.
        /// </summary>
		private int source_face_index = -1;
        ...
        public Build2DFaces(CSGBrush brush, int face_idx, CSGBrush brush_a) {
            ...
            // Convert 3D vertex points to 2D.
            Vector3[] points_3D = {
                brush_a.obj.Transform.Local.PointToLocal(brush.obj.Transform.World.PointToWorld(brush.faces[face_idx].vertices[0])),
                brush_a.obj.Transform.Local.PointToLocal(brush.obj.Transform.World.PointToWorld(brush.faces[face_idx].vertices[1])),
                brush_a.obj.Transform.Local.PointToLocal(brush.obj.Transform.World.PointToWorld(brush.faces[face_idx].vertices[2]))
            };
            plane = new PlaneCSG(points_3D[0], points_3D[1], points_3D[2]);
            to_3D = new Transform_to_2DFace();
            to_3D.Set_position(points_3D[0]);
        ...
        private int get_point_idx(Vector2 point) {
            for (int vertex_idx = 0; vertex_idx < vertices.Count; ++vertex_idx) {
                if ((vertices[vertex_idx].point - point).LengthSquared < vertex_tolerance) {
                    return vertex_idx;
                }
            }
            return -1;
            ...
            for (int sorted_idx = 1; sorted_idx < segments; ++sorted_idx) {
                ...
                for (int face_idx = 0; face_idx < faces.Count; ++face_idx) {
                    ...
                    for (int point_idx = 0; point_idx < degenerate_points.Count; ++point_idx) {
                        ...
                        bool existing = false;
                        for (int i = 0; i < 3; ++i) {
                            if ((face_vertices[i].point - point_2D).LengthSquared < vertex_tolerance) {
                                existing = true;
                                break;
                            }
                        }
                        ...
                        for (int face_edge_idx = 0; face_edge_idx < 3; ++face_edge_idx) {
                            ...
                            if ((point_2D - closest_point ).LengthSquared < vertex_tolerance) {
            ...
            for (int face_idx = 0; face_idx < faces.Count; ++face_idx) {
                ...
                for (int face_edge_idx = 0; face_edge_idx < 3; ++face_edge_idx) {
                    ...
                    for (int edge_point_idx = 0; edge_point_idx < 2; ++edge_point_idx) {
                        ...
                        if ((segment_points[edge_point_idx] - intersection_point).LengthSquared < vertex_tolerance) {
                            on_edge = true;
                            break;
                        }
                    }
                    ...
                    if (on_edge || segment_intersects_segment(segment_points[0], segment_points[1], edge_points[0], edge_points[1], ref intersection_point)) {
                        ...
                        if (((edge_points[0] - intersection_point).LengthSquared < vertex_tolerance) ||
                                ((edge_points[1] - intersection_point).LengthSquared < vertex_tolerance)) {
                            continue;
                        }
                        ...
                        if ((vertices[opposite_vertex_idx].point - closest_point).LengthSquared < vertex_tolerance) {
        ...
        private int insert_point(Vector2 point) {
            ...
            for (int face_idx = 0; face_idx < faces.Count; ++face_idx) {
                ...
                // Check if point is existing face vertex.
                for (int i = 0; i < 3; ++i) {
                    if ((face_vertices[i].point - point).LengthSquared < vertex_tolerance) {
                ...
                for (int face_edge_idx = 0; face_edge_idx < 3; ++face_edge_idx) {
                    ...
                    if ((point - closest_point).LengthSquared < vertex_tolerance) {
                        on_edge = true;
        ...
        public void insert(CSGBrush brush, int face_idx, CSGBrush brush_a = null) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if(brush_a==null){
                    ...
                    point_3D = brush_a.obj.Transform.Local.PointToLocal(brush.obj.Transform.World.PointToWorld(brush.faces[face_idx].vertices[i]));
                }

                if (plane.has_point(point_3D, CSGBrush.CMP_EPSILON)) {
                    // Point is in the plane, add it.
                    Vector3 point_2D = Vector3.VectorPlaneProject(point_3D , plane.normal);
                    ...
                    if(brush_a==null){
                        ...
                        next_point_3D = brush_a.obj.Transform.Local.PointToLocal(brush.obj.Transform.World.PointToWorld(brush.faces[face_idx].vertices[(i + 1) % 3]));
                    }
        ...
        public void addFacesToMesh(ref MeshMerge mesh_merge, bool from_b) {
            for (int face_idx = 0; face_idx < faces.Count; ++face_idx) {
                ...
                mesh_merge.add_face(points_3D, uvs, from_b, source_face_index);
            }
        }
    }
}
