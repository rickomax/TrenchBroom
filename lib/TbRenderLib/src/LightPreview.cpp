...
float parseIntensity(
  const mdl::Entity& entity, const std::optional<float>& extraIntensity)
{
  ...
  : m_map{map}
{
  ...
  // The revision has to change whenever the lighting would come out differently, which
  // is when the map is edited and, for a map with animated light styles, on every frame
  // of the animation. A map without any styled light looks the same from one frame to
  // the next, and leaving the frame out of its revision spares it a rebuild ten times a
  // second for no visible difference.
  const auto animated =
    std::ranges::any_of(m_lights, [](const auto& light) { return light.style > 0; });
  m_revision = (static_cast<uint64_t>(m_map.modificationCount()) << 32)
               | (animated ? m_styleFrame : 0u);
}
...
vm::vec3f LightPreview::lightingAt(
  const vm::vec3f& position, const vm::vec3f& normal, const mdl::BrushFace* face) const
{
  return lightingAt(position, normal, face, nullptr);
}
  ...
  auto visitor = kdl::overload(
    ...
    [&](auto&& thisLambda, mdl::EntityNode& entityNode) {
      ...
      if (kdl::cs::str_is_prefix(classname, "light"))
      {
        ...
        auto extraIntensity =
          colorValue.extraComponents.empty()
            ? std::optional<float>{}
