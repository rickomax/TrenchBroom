...
{
  // Brushes generated by a spline sweep are managed by the spline tool: selecting
  // and editing them individually would be undone by the next regeneration, so they
  // cannot be selected. Use the spline tool to edit the spline instead. That holds
  // wherever the sweep put them: in the spline's own entity, or in one of the entities
  // it generates to keep a template brush entity's class.
  if (const auto* entityNode = brushNode.entity();
      entityNode
      && (isSplineEntity(entityNode->entity())
          || isSplineGeneratedEntity(entityNode->entity())
          || isTerrainEntity(entityNode->entity())))
  {
    return false;
  }
