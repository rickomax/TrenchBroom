...
TEST_CASE_METHOD(EditorContextTest, "EditorContextTest.testSplineEntityBrushes")
{
  auto [entityNode, brushNode] = createTopLevelBrushEntity();

  REQUIRE(context.selectable(*brushNode));
  REQUIRE(context.selectable(*brushNode, brushNode->brush().face(0)));

  // Brushes generated by a spline sweep are managed by the spline tool and cannot be
  // selected.
  auto entity = entityNode->entity();
  entity.addOrUpdateProperty("_spline_point_0", "0 0 0 0 1 0");
  entityNode->setEntity(std::move(entity));

  CHECK_FALSE(context.selectable(*brushNode));
  CHECK_FALSE(context.selectable(*brushNode, brushNode->brush().face(0)));
}
